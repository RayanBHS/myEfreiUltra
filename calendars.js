...
(function () {
  ...
  function loadSettings() {
    try {
      const saved = localStorage.getItem('mye-calendars-settings');
      if (saved) {
  ...
  async function fetchPlanningForPeriod(date) {
    ...
    const endpoints = {
      ilab: `/api/rest/student/calendar/ilab/${state.campus}`,
      studio: `/api/rest/student/calendar/studio/${state.campus}`,
      association: `/api/rest/student/calendar/association/${state.campus}`,
      student: `/api/rest/student/calendar/student/${state.campus}`
    };
    
    // Explicit override requested by user
    if (state.campus === 'bdx') {
      endpoints.association = '/api/rest/student/calendar/association/paris';
    }

    let urlsToFetch = [];
    if (state.category === 'all') {
      urlsToFetch = Object.values(endpoints);
    } else {
      urlsToFetch = [endpoints[state.category]];
    }

    showSpinner();

    try {
      const promises = urlsToFetch.map(async u => {
        const fullUrl = new URL(u, window.location.origin);
        fullUrl.searchParams.set('startDate', startDateVal);
        fullUrl.searchParams.set('endDate', endDateVal);
        try {
          ...
          let injectedType = 'Evénements';
          if (u.includes('ilab')) injectedType = 'FabLab';
          else if (u.includes('studio')) injectedType = 'Studio';
          else if (u.includes('association')) injectedType = 'Asso';
          
          items.forEach(it => it._injectedType = injectedType);
          return items;
        } catch(e) { return []; }
      });
      ...
      await handlePlanningData(combined);
    } catch (e) {
  ...
  function mapEvent(raw) {
    ...
    let type = raw._injectedType || raw.type || raw.eventType || raw.lessonType || raw.category || raw.courseActivity || raw.sessionType || 'Cours';
    if (typeof type === 'object') type = type.name || type.label || type.code || JSON.stringify(type);
  ...
  function buildPageStructure() {
    const existing = document.getElementById('mye-calendars-container');
    if (existing) return;
        ...
        <div class="mac-cal-sidebar-section" style="height: 100%; display: flex; flex-direction: column; padding-bottom: 20px;">
          <div class="mye-sidebar-card" style="flex: 1; display: flex; flex-direction: column; margin-bottom: 0;">
            ...
            <div style="display:flex; flex-direction:column; gap:12px; padding-top:12px;">
              <div>
                <label style="font-size:12px; font-weight:600; color:#64748b; margin-bottom:4px; display:block;">Campus</label>
                <select id="mye-cal-campus" style="width:100%; padding:8px; border-radius:8px; border:1px solid #d2d2d7; background:#f5f5f7; cursor:pointer;">
                  <option value="paris" ${state.campus === 'paris' ? 'selected' : ''}>Paris</option>
                  <option value="bdx" ${state.campus === 'bdx' ? 'selected' : ''}>Bordeaux</option>
                </select>
              </div>
              <div>
                <label style="font-size:12px; font-weight:600; color:#64748b; margin-bottom:4px; display:block;">Catégorie</label>
                <select id="mye-cal-category" style="width:100%; padding:8px; border-radius:8px; border:1px solid #d2d2d7; background:#f5f5f7; cursor:pointer;">
                  <option value="all" ${state.category === 'all' ? 'selected' : ''}>Tous les événements</option>
                  <option value="ilab" ${state.category === 'ilab' ? 'selected' : ''}>FabLab</option>
                  <option value="studio" ${state.category === 'studio' ? 'selected' : ''}>Studio</option>
                  <option value="association" ${state.category === 'association' ? 'selected' : ''}>Association</option>
                  <option value="student" ${state.category === 'student' ? 'selected' : ''}>Evénements</option>
                </select>
              </div>
            </div>
    ...
    const mainContainer = document.getElementById('mye-calendars-container');
    ...
    document.getElementById('mye-cal-campus').addEventListener('change', (e) => {
      state.campus = e.target.value;
      fetchPlanningForPeriod(state.currentDate);
    });
    
    document.getElementById('mye-cal-category').addEventListener('change', (e) => {
  ...
  function normalizeType(type) {
    if (!type) return 'Evénements';
    const t = type.toLowerCase();
    
    if (t.includes('fablab') || t.includes('ilab')) return 'FabLab';
    if (t.includes('studio')) return 'Studio';
    if (t.includes('asso')) return 'Asso';
    return 'Evénements';
  }
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
      if (window.location.pathname.includes('/portal/common/calendars')) waitAndInit();
    });
  } else {
    if (window.location.pathname.includes('/portal/common/calendars')) waitAndInit();
  }
